 ...
 *   exePath       {string|null}   Path to a bundled installer exe, relative to the
 *                                 ModManagerApp root.
 *   exeArgs       {string[]}      Optional arguments passed to the installer exe.
 *
 * Status rules:
...
const REQUIREMENT_DEFS = [
  {
    ...
    checkPaths: ["dsound.dll", "UniversalSigBypasser.asi"],
    versionCheck: null,
    url: null,
    zipPath: "install/SigBypasser_v1.2.zip",
    installPath: ".",
    exePath: null,
    exeArgs: [],
  },
  {
    ...
    checkPaths: ["dwmapi.dll", "ue4ss"],
    versionCheck: null,
    url: null,
    zipPath: "install/ue4ss.zip",
    installPath: ".",
    exePath: null,
    exeArgs: [],
  },
  {
    ...
    checkPaths: ["ue4ss/Mods/DataTablePatcher"],
    versionCheck: {
      jsonPath: "ue4ss/Mods/DataTablePatcher/manifest.json",
      field: "version",
      required: ">=3",
    },
    url: null,
    zipPath: "install/DataTablePatcher_v3.zip",
    installPath: "ue4ss/Mods",
    exePath: null,
    exeArgs: [],
  },
 ...
 * @returns {{ id, name, description, installed, statusDetail, url, zipPath, installPath, exePath, exeArgs }}
 */
function checkRequirement(req, gameExePath) {
  ...
  if (!gameDir || (!hasPathChecks && !hasVersionCheck)) {
    return {
      ...
      exePath: req.exePath ?? null,
      exeArgs: Array.isArray(req.exeArgs) ? req.exeArgs : [],
    };
  }
...
/**
 * Runs a requirement installer exe and waits for it to finish.
 *
 * @param {object} req         Requirement definition.
 * @param {string} appDir      Absolute path to the ModManagerApp root.
 * @returns {Promise<void>}
 */
async function installRequirementFromExe(req, appDir) {
  if (!req.exePath) {
    throw new Error(`Requirement "${req.id}" does not have an exePath field.`);
  }

  const exeAbsPath = path.join(appDir, req.exePath);
  if (!existsSync(exeAbsPath)) {
    throw new Error(`Installer executable not found: ${exeAbsPath}`);
  }

  const args = Array.isArray(req.exeArgs) ? req.exeArgs : [];

  await new Promise((resolve, reject) => {
    const child = spawn(exeAbsPath, args, {
      cwd: path.dirname(exeAbsPath),
      windowsHide: false,
      stdio: "ignore",
    });
...
/**
 * Installs a requirement using the configured install method.
 * Supports:
 *   - zipPath + installPath
 *   - exePath (+ optional exeArgs)
 *
 * @param {object} req
 ...
 * @returns {Promise<void>}
 */
async function installRequirement(req, gameExePath, appDir) {
  if (req.zipPath && req.installPath) {
    await installRequirementFromZip(req, gameExePath, appDir);
    return;
  }
  if (req.exePath) {
    await installRequirementFromExe(req, appDir);
    return;
  }
  throw new Error(
    `Requirement "${req.id}" has no supported install configuration (zipPath/installPath or exePath).`,
  );
}
