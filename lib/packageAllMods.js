...
async function patchCookedDatatables(modManagerDir, outputDir, reporter = null) {
  ...
  const { patchModManagerDirectory } = await import(pathToFileURL(patcherIndexPath).href);
  ...
  if (summary.errors.length > 0) {
    const details = summary.errors.map((e) => `${e.table}: ${e.error}`).join("\n");
    throw new Error(`Cooked datatable patch failed:\n${details}`);
  }

  if (summary.skipped.length > 0) {
    throw new Error(
      `Missing cooked base tables for: ${summary.skipped.join(", ")}. ` +
  ...
  reporter?.({
    type: "log",
    stream: "info",
    message: `Patched ${summary.patched.length} cooked datatable(s)`,
  });

  return { patched: summary.patched.length };
}
 ...
 *  30  – Patch cooked DataTables into staging (CookedDatatablePatcher)
...
async function packageAllMods(modsFolder, enabledModIds = [], reporter = null, gameExePath = null) {
  ...
  if (combinedTables > 0) {
    ...
    reporter?.({ type: "progress", step: "Patching cooked DataTables", progress: 30 });
